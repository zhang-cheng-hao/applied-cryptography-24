...
class ModbusTcpClient(BaseModbusClient):
    ...
    def __init__(self, host='127.0.0.1', port=Defaults.Port,
        framer=ModbusSocketFramer, private_key=None, public_key=None, sm4_key=None, trusted_key=None, **kwargs):
        """ Initialize a client instance
        ...
        BaseModbusClient.__init__(self, framer(ClientDecoder(), self, private_key=private_key, public_key=public_key,
                                               sm4_key=sm4_key,
                                               trusted_key=trusted_key), **kwargs)
    ...
    def connect(self):
        ...
        try:
            ...
            # 客户端生成随机密钥 R3
            R3 = secrets.token_bytes(16)
            K = secrets.token_bytes(16).hex()
            print("[发送] 客户端生成随机 16 字节密钥 R3: {}".format(R3))

            # 使用服务器公钥加密 R3
            R3_encode = cpr.encrypt(R3)
            print("[发送] 使用服务器公钥加密 R3: {}".format(R3_encode))

            # 对加密后的 R3 进行签名
            R3_encode_sign = self.framer.crypter.sign(R3_encode, K)
            print("[发送] 用客户端私钥签名加密后的 R3: {}".format(R3_encode_sign))

            # 将加密和签名结果打包发送
            send_agt = R3_encode + R3_encode_sign.encode("utf-8")
            print("[发送] 打包发送加密和签名结果: {}".format(send_agt))
            time2 = time.time()

            # 发送数据
            self.socket.send(send_agt)

            # 接收服务器的响应
            rcv_agt = self.socket.recv(1024)
            time3 = time.time()

            # 解密服务器响应
            agt_msg = self.framer.crypter.decrypt(rcv_agt).decode('utf-8')
            if agt_msg[0] == 'S':
                self.framer.sm4_key = R3.hex()
            print("[接收] 密钥协商响应消息: {}".format(agt_msg))
            print("[结果] 协商完成, SM4 密钥 = {}".format(self.framer.sm4_key))

            time4 = time.time()
            print("密钥协商完成，用时 {:.3f} 秒".format((time4 - time1) - (time3 - time2)))
            print("----------- 密钥协商结束 ---------------------------------------------------")
